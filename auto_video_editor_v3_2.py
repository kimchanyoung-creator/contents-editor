...
# 강제 재설치로 버전 충돌 방지
!pip uninstall -y scipy > /dev/null 2>&1
...
# 2. scipy 호환성 패치 (중요!)
# ============================================================
# scipy 1.12+ 에서는 scipy.signal.hann이 scipy.signal.windows.hann으로 이동됨
# librosa가 내부적으로 scipy.signal.hann을 사용하므로 호환성 패치 필요
import scipy.signal
import scipy.signal.windows

# 누락된 함수들을 signal 네임스페이스에 추가
if not hasattr(scipy.signal, 'hann'):
    scipy.signal.hann = scipy.signal.windows.hann
if not hasattr(scipy.signal, 'hamming'):
    scipy.signal.hamming = scipy.signal.windows.hamming

print("✅ scipy 호환성 패치 완료!\n")
...
class UploadManager:
    ...
    def show(self):
        ...
        <div style='background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding:20px; border-radius:15px; margin-bottom:20px;'>
            <h1 style='color:white; margin:0;'>🎬 AI 자동 비디오 편집기 v3.2</h1>
